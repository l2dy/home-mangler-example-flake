...
let
  ...
  loop = dir:
    let
      toKeyVal = basename: type:
        let path = dir + "/${basename}";
        in {
          name = builtins.replaceStrings [ ".nix" ] [ "" ] basename;

          value = if type == "regular" then
            callPackage path { }
          else if type == "directory" then
            let package = path + "/package.nix";
            in if builtins.pathExists package then
              callPackage package { }
            else
              loop path
          else
    ...
    in builtins.listToAttrs (lib.mapAttrsToList toKeyVal
      (lib.filterAttrs supported (builtins.readDir dir)));
in loop (./. + "/${directory}")
